...
export class TopologicalSorter {
    ...
    if (cycles.length > 0) {
      for (const cycle of cycles) {
        conflicts.push({
          ...
          pluginId: cycle[0] as PluginId,
          dependencyId: cycle[cycle.length - 1] as PluginId,
          description: `Circular dependency detected: ${cycle.join(' -> ')}`,
          severity: 'error' as const,
          suggestedResolution: 'Remove or restructure dependencies to break the cycle',
          conflictingPlugins: cycle.map(id => id as PluginId),
          details: { cycle: cycle.map(id => id as PluginId) },
        });
      }
  ...
  static sortWithLoadOrder(
    ...
    for (const spec of loadOrderSpecs) {
      try {
        ...
        conflicts.push({
          type: ConflictType.LOAD_ORDER_CONFLICT,
          pluginId: spec.plugin as PluginId,
          dependencyId: spec.plugin as PluginId,
          description: `Failed to apply load order constraints for '${spec.plugin}': ${error}`,
          severity: 'error' as const,
          suggestedResolution: 'Check that load order constraints reference valid plugins',
        });
      }
    }
    ...
    if (cycles.length > 0) {
      for (const cycle of cycles) {
        conflicts.push({
          ...
          pluginId: cycle[0] as PluginId,
          dependencyId: cycle[cycle.length - 1] as PluginId,
          description: `Load order constraints created circular dependency: ${cycle.join(' -> ')}`,
          severity: 'error' as const,
          suggestedResolution: 'Adjust load order constraints to avoid cycles',
          conflictingPlugins: cycle.map(id => id as PluginId),
          details: { cycle: cycle.map(id => id as PluginId) },
        });
      }
      ...
      for (const dependency of Array.from(node.dependencies)) {
        ...
        if (depPos === undefined) {
          conflicts.push({
            ...
            dependencyId: dependency as PluginId,
            description: `Plugin '${node.plugin.name}' depends on '${dependency}' which is not in the initialization order`,
            severity: 'error' as const,
            suggestedResolution: `Add '${dependency}' to the initialization order before '${node.plugin.name}'`,
          });
          ...
          conflicts.push({
            ...
            pluginId: node.plugin.id,
            dependencyId: dependency as PluginId,
            description: `Plugin '${node.plugin.name}' must be initialized after its dependency '${dependency}'`,
            severity: 'error' as const,
            suggestedResolution: `Move '${dependency}' before '${node.plugin.name}' in the initialization order`,
          });
        }
      }
    }
  ...
  private static kahnsAlgorithm(graph: DependencyGraph): readonly string[] {
    ...
    for (const [plugin, degree] of Array.from(inDegree.entries())) {
      if (degree === 0) {
        queue.push(plugin);
      }
    }
    ...
    for (const target of spec.loadBefore) {
      if (!graph.getNode(target)) {
        conflicts.push({
          ...
          dependencyId: target as PluginId,
          description: `Load order constraint references unknown plugin '${target}'`,
          severity: 'error' as const,
          suggestedResolution: `Remove reference to '${target}' or add it to the graph`,
        });
        continue;
      }
