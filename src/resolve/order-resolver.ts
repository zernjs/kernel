...
import type { ResolveInput, PluginInstance } from '@types';
...
export function resolvePluginOrder(input: ResolveInput): PluginInstance[] {
  ...
  for (const p of plugins) {
    ...
    for (const depCtor of deps) {
      ...
      if (!byName.has(dep.metadata.name)) continue; // validação de ausência logo abaixo
      graph.addEdge(dep.metadata.name, p.metadata.name, 'dep');
    }
  }

  // user order (strong, but below deps)
  ...
  // sort deterministically by registration order
  const nameToIndex = new Map<string, number>();
  plugins.forEach((p, i) => nameToIndex.set(p.metadata.name, i));
  const prefer = (a: string, b: string): number => nameToIndex.get(a)! - nameToIndex.get(b)!;
  ...
  const orderedNames = sortedRes.value;
  const ordered = orderedNames.map(n => byName.get(n)!).filter(Boolean);

  // validação: deps ausentes e versões
  for (const p of plugins) {
    const declared = (
